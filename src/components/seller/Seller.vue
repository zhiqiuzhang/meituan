<template>
  <div class="seller" ref="sellerView">
    <div class="seller-wrapper">
        ...
        <!-- 图片 -->
        <div class="pics-wrapper" v-if="seller.poi_env" ref="picWrapper">
          <ul class="pics-list" ref="picList">
            <li
              class="pics-item"
              ref="picItem"
              v-for="(imgItem, index) in seller.poi_env.thumbnails_url_list"
              :key="index"
              >
              <img :src="imgItem">
            </li>
        ...
        if(response.code == 0){
          ...
          this.$nextTick(() => {
          	if(this.seller.poi_env.thumbnails_url_list){
          		let imgItemW = this.$refs.picItem[0].clientWidth;
          		let marginR = 11;
          		let width = (imgItemW + marginR) * this.seller.poi_env.thumbnails_url_list.length;
